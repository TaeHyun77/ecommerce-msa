...
public class RestClientConfig {
    ...
    public RestClient paymentServiceRestClient(
        ...
        return builder
                .baseUrl(baseUrl)
                // payment-service가 토스 승인 응답을 최대 60초 기다리므로 그보다 길게 잡는다
                // 짧으면 payment-service는 결과를 확정했는데 이쪽만 결과 불명이 되어 복구를 기다려야 한다
                .requestFactory(requestFactory(Duration.ofSeconds(1), Duration.ofSeconds(70)))
                // 결과 불명(503)을 포함한 5xx는 승인이 처리됐는지 알 수 없으므로 결과 불명으로 다룬다
                .defaultStatusHandler(
                        HttpStatusCode::is5xxServerError,
                        (request, response) -> {
                            throw new PaymentResultUnknownException("payment-service 응답 status=" + response.getStatusCode().value());
                        }
                )
                .defaultStatusHandler(
                        HttpStatusCode::is4xxClientError,
